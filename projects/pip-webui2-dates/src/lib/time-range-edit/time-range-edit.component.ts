...
export class PipTimeRangeEditComponent implements OnInit, AfterViewInit {
    ...
    @Input() public startLabel = 'DATES.START_TIME';
    @Input() public endLabel = 'DATES.END_TIME';
    @Input() intervalInMinutes = 30;
    ...
    private getTimeByDate(date: Date) {
        ...
        const intervalInMs = this.intervalInMinutes * 60 * 1000;
    ...
    private checkRange(start: boolean = true) {
        if (this.startDate && this.endDate) {
            if (this.startDate.getTime() > this.endDate.getTime()) {
                if (start) {
                    this.setEndValuesByDate(new Date(this.startDate.getTime() + this.range));
                } else {
                    this.setStartValuesByDate(new Date(this.endDate.getTime() - this.range));
                }
